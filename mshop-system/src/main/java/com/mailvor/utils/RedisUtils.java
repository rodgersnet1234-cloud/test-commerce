...
public class RedisUtils {
    ...
    public Map<String, Object> scan(String pattern, Integer count, String cursorId) {
        if (cursorId == null) {
            cursorId = "0";
        }

        RedisScript<List> redisScript = RedisScript.of("return redis.call('scan',KEYS[1],'match',ARGV[1],'count',ARGV[2])", List.class);
        RedisSerializer serializer = this.redisTemplate.getStringSerializer();
        Set<Object> valueList = new HashSet();

        do {
            List execList = (List)this.redisTemplate.execute(redisScript, serializer, serializer, Collections.singletonList(cursorId), new Object[]{pattern, count.toString()});

            assert execList != null;

            cursorId = String.valueOf(execList.get(0));
            List<String> keyList = (List)execList.get(1);
            List<Object> findList = this.redisTemplate.opsForValue().multiGet(new HashSet(keyList));
            valueList.addAll(findList);
            if (valueList.size() >= count) {
                Map<String, Object> res = new HashMap();
                res.put("cursorId", cursorId);
                res.put("list", valueList);
                return res;
            }
