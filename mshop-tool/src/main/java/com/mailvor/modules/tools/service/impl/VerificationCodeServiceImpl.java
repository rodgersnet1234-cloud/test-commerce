...
import com.mailvor.modules.tools.service.mapper.VerificationCodeMapper;
import com.mailvor.modules.tools.domain.VerificationCode;
import com.mailvor.modules.tools.domain.vo.EmailVo;
import com.mailvor.modules.tools.service.VerificationCodeService;
...
public class VerificationCodeServiceImpl extends BaseServiceImpl<VerificationCodeMapper, VerificationCode>  implements VerificationCodeService {
    ...
    public EmailVo sendEmail(VerificationCode code) {
        ...
        if(verificationCode == null){
            code.setCode(RandomUtil.randomNumbers (6).toUpperCase(Locale.ROOT));
            content = template.render(Dict.create().set("code",code.getCode()));
            emailVo = new EmailVo(Collections.singletonList(code.getValue()),"mshop后台管理系统",content);
            this.save(code);
            timedDestruction(code);
        // 存在就再次发送原来的验证码
        } else {
            content = template.render(Dict.create().set("code",verificationCode.getCode()));
            emailVo = new EmailVo(Collections.singletonList(verificationCode.getValue()),"mshop后台管理系统",content);
        }
        return emailVo;
    }
