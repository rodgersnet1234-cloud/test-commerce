...
import static com.mailvor.utils.FileUtil.transformStyle;
...
public class CreatShareProductService {
     ...
     */
    public MwStoreOrderQueryVo handleQrcode(MwStoreOrderQueryVo storeOrder, String path){
     ...
     * @return url
     */
    public String getSpreadUrl(String from, MwUser userInfo, String siteUrl,
        ...
        if(StrUtil.isNotBlank(from) && AppFromEnum.APP.getValue().equals(from)){
            ...
            MwSystemAttachment attachmentT = systemAttachmentService.getInfo(spreadPicName);
     ...
     * @return url
     */
    public String getBargainPosterUrl(Long bargainId, MwUser userInfo, String siteUrl,
                                      String apiUrl, String path, String from){
        Long uid = userInfo.getUid();
        MwStoreBargain storeBargainQueryVo = storeBargainService
                .getById(bargainId);
     ...
     * @return url
     */
    public String getPinkPosterUrl(Long pinkId, MwUser userInfo, String siteUrl,
                                   String apiUrl, String path, String from){
        Long uid = userInfo.getUid();
        MwStorePink storePink = storePinkService.getById(pinkId);
        if(ObjectUtil.isNull(storePink)) {
            throw new MshopException("拼团不存在");
        }
        MwStoreCombination storeCombination = storeCombinationService.getById(storePink.getCid());
        if(ObjectUtil.isNull(storeCombination)) {
            throw new MshopException("拼团产品不存在");
        }
...
//                        FileUtil.file(picImage));
     ...
     */
    public  String creatProductPic(MwStoreProduct productDTO, String shareCode, String spreadPicName, String spreadPicPath, String apiUrl) throws IOException, FontFormatException {
        MwSystemAttachment attachmentT = systemAttachmentService.getInfo(spreadPicName);
        String spreadUrl = "";
        if(ObjectUtil.isNull(attachmentT)){
            //创建图片
